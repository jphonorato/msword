...
#ifdef OPUS_X64
#define JcFromTbdByte(b)  ((unsigned char)(b) & 0x07)
#define TlcFromTbdByte(b) (((unsigned char)(b) >> 3) & 0x07)
#else
#define JcFromTbdByte(b)  (((struct TBD)(b)).jc)
#define TlcFromTbdByte(b) (((struct TBD)(b)).tlc)
#endif
...
CMB * pcmb;
{
	...
	if (vptdsd->itbdMac == 0) 
		...
		bltbyte(&vptdsd->rgtbd[0], &vptdsd->tbdCur, sizeof(struct TBD));
		return fTrue;
		}
}
...
LErr:
	...
	default:
		if (fAllowBlank && 
				JcFromTbdByte(vptdsd->rgtbd[vptdsd->itbdCur]) ==
				vptdsd->tbdCur.jc &&
				TlcFromTbdByte(vptdsd->rgtbd[vptdsd->itbdCur]) ==
				vptdsd->tbdCur.tlc)
...
CMB * pcmb;
{
	...
	if (vptdsd->itbdCur == itbdNil || vptdsd->itbdCur == itbdNew ||
		...
		*/
		...
		{
		if (JcFromTbdByte(vptdsd->rgtbd[vptdsd->itbdCur]) !=
				JcFromI(ValGetTmc(tmcAlignment)))
			{
			SetTmcVal(tmcAlignment,
					IFromJc(JcFromTbdByte(vptdsd->rgtbd[vptdsd->itbdCur])));
			}

		if (TlcFromTbdByte(vptdsd->rgtbd[vptdsd->itbdCur]) !=
				TlcFromI(ValGetTmc(tmcLeader)))
			{
			SetTmcVal(tmcLeader,
					IFromTlc(TlcFromTbdByte(vptdsd->rgtbd[vptdsd->itbdCur])));
			}
		}
}
...
CMB * pcmb;
{
	...
	if (itbd != itbdNew)
		...
		if (vptdsd->itbdMac == 0)
			...
			bltbyte(&vptdsd->rgtbd[vptdsd->itbdCur], &vptdsd->tbdCur,
					sizeof(struct TBD));

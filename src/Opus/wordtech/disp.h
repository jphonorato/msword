...
struct WWD
	...
	int	fExternal;
#ifdef OPUS_X64
	struct PLDR **hpldrBack;  /* native handle; nil distinguishes WWD */
#else
...
#ifdef OPUS_X64
int DlWhereDocCp(int ww, int doc, CP cp, int fEnd,
		struct PLDR ***phpldr, int *pidr, CP *pcpFirst,
		BOOL *pfChangedPage, BOOL fInner);
#endif

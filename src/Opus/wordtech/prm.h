...
struct PRM
		...
		union
				...
				struct
						{
#ifdef OPUS_X64
						int     fComplexPrc : 1; /* same overlay bit */
#else
...
#ifdef OPUS_X64
#define HprcFromPprmComplex(pprm) ((struct PRC **)(AssertX((pprm)->fComplex),OpusPrcHandleFromToken((pprm)->cfgrPrc)))
#elif defined(WIN)
...
#ifdef OPUS_X64
#define cbPRCBase ((int)offsetof(struct PRC, grpprl))
#else
#define cbPRCBase 6
#endif
/*
...
struct PCVH /* position code operand for sprmPPc */
	...
	union
		...
		struct
			{
			unsigned short : 3;
			unsigned short fVert : 1;
			unsigned short pcVert : 2;
			unsigned short pcHorz : 2;
			unsigned short : 8;
			};
		struct
			{

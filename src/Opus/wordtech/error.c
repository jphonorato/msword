...
#undef StKey
#undef St
#define StKey(s, key) ((const CHAR *)(s))
#define St(s) ((const CHAR *)(s))
#endif
...
#undef StKey
#undef St
#define StKey(s, key) StringMap(s, 1, 1)
#define St(s) StringMap(s, 1, 1)

static int CchEmdSt(iemd)
int iemd;
	{
	return CchSz(rgemd[iemd].st) - 1;
	}

static CopyEmdSt(iemd, st)
int iemd;
CHAR *st;
	{
	int cch = CchEmdSt(iemd);
	st[0] = (CHAR)cch;
	bltbx(rgemd[iemd].st, st + 1, cch);
	}
#endif
...
#ifdef OPUS_X64
	bltbx((CHAR FAR *)rgemd[iemd].st, (CHAR FAR *)sz, cch);
#else
...
#ifdef OPUS_X64
csconst CHAR stNoMemPrefix[] = "\025Not enough memory to ";
#else
...
#ifdef OPUS_X64
	bltbx(rgemd[iemd].st, szMsg + cch, CchEmdSt(iemd));
	cch += CchEmdSt(iemd);
#else
...
int eid;
{
	...
	if (!vmerr.fErrorAlert)
	    ...
	    Assert(
#ifdef OPUS_X64
			CchEmdSt(iemd) > 0 && CchEmdSt(iemd) < cchMaxSz
#else
			rgemd[iemd].st[0] > 0 && rgemd[iemd].st[0] < cchMaxSz
#endif
			);
#ifdef OPUS_X64
	    CopyEmdSt(iemd, szMsg);
#else
...
#ifdef OPUS_X64
		{
		const char *szReport = OpusX64LastReport();
		if (szReport != NULL && *szReport != 0 &&
				lstrlenA(szMsg) + lstrlenA(szReport) + 21 < cchMaxSz)
			{
			lstrcatA(szMsg, "\r\n\r\nStartup stage: ");
			lstrcatA(szMsg, szReport);
			}
		}
#endif

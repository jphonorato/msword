...
/* initialize run table */
	vifmaMac = 0;
	vbchrMac = 0;
	/* OverhangChr ignores its index while the run table is empty.  The
	 * segmented compiler therefore tolerated the historical uninitialized
	 * argument; initialize it for checked native AMD64 calls. */
	bchrPrev = 0;
	bchrChp = -1;
...
#ifdef WIN
#ifdef OPUS_X64
		jcTab = ((unsigned char)vpapFetch.rgtbd[itbd]) & 7;
#else
...
#ifdef WIN
#ifdef OPUS_X64
			(((unsigned char)vpapFetch.rgtbd[itbd]) & 7) == jcBar)
#else

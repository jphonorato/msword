...
/* date spec */
	...
	while (iCatSrhSpec < iCatSrhSpecMac)
		...
		SetCabL(hcab, pdttm->lDttm, rgIagCatSrh[iCatSrhSpec++]);
		pdttm++;
		}
...
/* dates */
	...
	while (iCatSrhSpec < iCatSrhSpecMac)
		{
		dttm2.lDttm = LFromCab(hcab, 
				(fElActive ? rgIagCatSrhEl : rgIagCatSrh)[iCatSrhSpec]);
		if (pdttm1->lDttm != 0L && CompareDTTMDate(*pdttm1, dttm2) != 0)
			fNarrowSearch = fFalse;
	...
	if (dlm == dlmTerm && tmc == tmcOK)
		...
		for (iCatSrhSpec = 0; iCatSrhSpec < iCatSrhSpecMac; iCatSrhSpec++)
			...
			if (iCatSrhSpec < iCatSrhSzSpecMac)
				...
				{
				dttmFrom.lDttm = LFromCab(hcab, rgIagCatSrh[iCatSrhSpec]);
				dttmTo.lDttm = LFromCab(hcab, rgIagCatSrh[iCatSrhSpec+1]);
				if (dttmTo.lDttm != 0L && CompareDTTMDate(dttmFrom, dttmTo) > 0)
					{ /* From later than To */

...
static csconst BYTE mpelrelv[] =
{
	elvNil, elvNum, elvInt, elvSd, elvArray, elvNil};


static csconst NUM numOne = {
...
BOOL fSuper;
{
	...
	for (fDoCall = !(elr == elrDialog && !fRecordArg); fDoCall < 2;
		...
		HCAB hcabArg;
#ifdef OPUS_X64
		OPUS_ELX_NATIVE_ARG rgNativeArgs[celpMax + 1];
		int cNativeArgs = 0;
#endif
		...
		for (ielp = 0; ielp < ele.celpMac; ielp++)
			...
			if (ielp >= cevPositional)
				...
				case elpHst:
#ifdef OPUS_X64
					rgNativeArgs[cNativeArgs].kind =
							ele.rgelp[ielp] == elpHst ?
							opusElxArgPointer : opusElxArgInt;
					if (ele.rgelp[ielp] == elpHst)
						rgNativeArgs[cNativeArgs].pointer = NULL;
					else
						rgNativeArgs[cNativeArgs].integer = 0;
					cNativeArgs++;
#else
					StackW(0);
#endif
				...
				case elpHpsd:
#ifdef OPUS_X64
					rgNativeArgs[cNativeArgs].kind = opusElxArgPointer;
					rgNativeArgs[cNativeArgs++].pointer = NULL;
#else
					StackL(hpNil);
#endif
				...
				case elpHpsdNum:
					...
					rgNativeArgs[cNativeArgs].kind = opusElxArgPointer;
					rgNativeArgs[cNativeArgs++].pointer = NULL;
					rgNativeArgs[cNativeArgs].kind = opusElxArgNum;
					BLTBCX(0, rgNativeArgs[cNativeArgs].num, sizeof(NUM));
					cNativeArgs++;
					cwPushed += 2 + cbNUM / sizeof(int);
					break;
#else
					StackL(hpNil);
					cwPushed += 2;
			...
			case elpInt:
#ifdef OPUS_X64
				rgNativeArgs[cNativeArgs].kind = opusElxArgInt;
				rgNativeArgs[cNativeArgs++].integer = hpev->w;
#else
				StackW(hpev->w);
#endif
				cwPushed += 1;
...
#ifdef OPUS_X64
				rgNativeArgs[cNativeArgs].kind = opusElxArgPointer;
				rgNativeArgs[cNativeArgs++].pointer = hpev->hst;
#else
				StackW(hpev->hst);
#endif
				cwPushed += 1;
...
#ifdef OPUS_X64
				rgNativeArgs[cNativeArgs].kind = opusElxArgPointer;
				rgNativeArgs[cNativeArgs++].pointer = &hpev->sd;
#else
				StackL(&hpev->sd);
#endif
				cwPushed += 2;
			...
			case elpHpsdNum:
				if (elvArg == elvSd)
					...
					rgNativeArgs[cNativeArgs++].pointer = &hpev->sd;
					rgNativeArgs[cNativeArgs].kind = opusElxArgNum;
					BLTBH(&numT, rgNativeArgs[cNativeArgs].num, sizeof(NUM));
					cNativeArgs++;
#else
					StackL(&hpev->sd);
					StackNum(numT);
		...
		case elrVoid:
			if (fElElx)
				{
#ifdef OPUS_X64
				if (!OpusInvokeElx((void *)ele.pfn, elrVoid,
						cNativeArgs, rgNativeArgs, NULL))
		...
		case elrNum:
				{
#ifdef OPUS_X64
				if (!OpusInvokeElx((void *)ele.pfn, elrNum,
						cNativeArgs, rgNativeArgs, hpvalReturn))
					RtError(rerrInternalError);
#else
		...
		case elrSd:
#ifdef OPUS_X64
			if (!OpusInvokeElx((void *)ele.pfn, elr,
					cNativeArgs, rgNativeArgs, hpvalReturn))
				RtError(rerrInternalError);
#else

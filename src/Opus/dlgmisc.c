...
PromptUserName()
{
	...
	/* The Win16 SDM dialog template is not directly consumable by Win32.
	   Seed the same original PREF fields from the current Windows account so
	   first-run startup can continue until the SDM template is translated. */
	cch = GetEnvironmentVariableA("USERNAME", sz, sizeof(sz));
	if (cch == 0 || cch >= sizeof(sz))
		lstrcpynA(sz, "Windows User", sizeof(sz));

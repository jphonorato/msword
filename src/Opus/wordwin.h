...
#ifdef OPUS_X64
#define FNoHeap(h)		((uintptr_t)(h) == (uintptr_t)hNil)
#else
...
#ifdef OPUS_X64
#define LowWord(l) ((int)((uintptr_t)(l) & 0xffffu))
#define HighWord(l) ((int)(((uintptr_t)(l) >> 16u) & 0xffffu))
#define MakeLong(lo,hi) ((long)(((unsigned)(lo) & 0xffffu) | \
		((unsigned long)((unsigned)(hi) & 0xffffu) << 16u)))
#define BLTB(p1,p2,cb) bltbyteNat((p1), (p2), (cb))
#define BLTBX(lp1,lp2,cb) bltbxNat((lp1), (lp2), (cb))
#define BLTCX(w,lpw,cw) bltcxNat((w), (lpw), (cw))
#define BLTBCX(b,lpb,cb) bltbcxNat((b), (lpb), (cb))
#define BLTBV(p1,p2,cb) ((CHAR *)bltbyteNat((p1), (p2), (cb)) + (cb))
#define BLTBXV(lp1,lp2,cb) ((CHAR FAR *)bltbxNat((lp1), (lp2), (cb)) + (cb))
#else
uop void _UOP_VOID();
uop int _UOP_INT();
uop long _UOP_LONG();
...
#ifdef OPUS_X64
#define ALLOCA(cb)  _alloca((size_t)(cb))
#else
...
#ifdef OPUS_X64
#define bltbyte(pbFrom, pbTo, cb) bltbyteNat((pbFrom), (pbTo), (cb))
#define bltb bltbyte
#define bltbx(lpbFrom, lpbTo, cb) bltbxNat((lpbFrom), (lpbTo), (cb))
#define bltbh(hpbFrom, hpbTo, cb) BLTBH((hpbFrom), (hpbTo), (cb))
#define bltbcx(bFill, lpbTo, cb) bltbcxNat((bFill), (lpbTo), (cb))
#define bltcx(wFill, lpwTo, cw) bltcxNat((wFill), (lpwTo), (cw))
#else
...
#ifdef OPUS_X64
/* Preserve Win16 AnsiLower's character-in-the-low-word convention without
 * narrowing a native pointer through a 32-bit unsigned long. */
#define ChLower(ch) \
	((CHAR)(ULONG_PTR)CharLowerA((LPSTR)(ULONG_PTR)(unsigned char)(ch)))
#else
...
#ifdef OPUS_X64
static __inline BOOL OpusPtInRect(struct PT pt, const struct RC *prc)
	{
	POINT point = { pt.xp, pt.yp };
	return PtInRect((const RECT *)prc, point);
	}
#define FPtInRect(pt, prc)	OpusPtInRect((pt), (prc))
#else

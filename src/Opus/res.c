...
#endif /* !OPUS_X64 -- built as a standalone original routine on x64 */
...
#endif /* !OPUS_X64: segmented/Win16 heap implementation */

#ifdef OPUS_X64
/* Keep the original size-query algorithm while routing storage through the
 * native handle runtime declared by heap.h. */
uns CbOfHplc(hplc)
struct PLC **hplc;
{
	AssertH(hplc);
...
int ww;
{
	...
	if (pwwd->fPageView || ww == wwPreview || pwwd->idrMac == 0)
		return(pmwd->doc);
#ifdef OPUS_X64
	pdr = PdrFetchAndFree((struct PLDR **)HwwdWw(ww), 0, &drfFetch);
#else

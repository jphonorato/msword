...
#ifdef WIN23
#ifdef OPUS_X64
WNDPROC		lpfnSdmWndProc;
WNDPROC		lpfnIconDlgWndProc;
#else
...
#ifdef OPUS_X64
NATIVE UINT_PTR WRefFromHwndIb(hwnd)
#else
...
#ifdef OPUS_X64
LRESULT CALLBACK IconBarWndProc(HWND hwnd, UINT wMessage, WPARAM wParam, LPARAM lParam)
#else
...
#endif
{
	...
	case WM_LBUTTONDBLCLK:
			...
			if (!FMouseHitIibb(hibs, 
					iibb = IibbFromHibsPt(hibs, pt), &pibb))
...
#ifdef WIN23
#ifdef OPUS_X64
LRESULT CALLBACK IconDlgWndProc(HWND hwnd, UINT message, WPARAM wParam,
		LPARAM lParam)
#else
...
#endif
{
	...
	rval = CallWindowProc((WNDPROC)lpfnSdmWndProc, hwnd, message, wParam, lParam);
	if (message == WM_ERASEBKGND || message == WM_PAINT)
		{

...
SZ szName;
{
	int tk;
	int irngDlg, crngDlg;
	/* A DTTM consumes a second synthetic CAB range entry.  The 16-bit
	 * compiler happened to present a zeroed slot here; make that historical
	 * assumption explicit for deterministic native generation. */
	RNG rgrngDlg [64] = { 0 };
	CHAR * pch;
	F fAllowNinch;
